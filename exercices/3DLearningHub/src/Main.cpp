...
std::vector<float>circleCenter{
  ...
  int main()
  {
	...
	//set models
	setLightCube(lightSourcesShader, cubeEdge, lightCubeObjects);
	updateLightsCubePos(lightCubeObjects);
	...
	auto drawSceneWithEffect = [&fbo,&lightSourcesShader,&lightCube,&lightCubeObjects,&woodBoxShader,&outlineShader,&skyBox,&woodCube,&skyboxShader,&quad,&postProcessShader,&newFrame]()
		{
			//BindFbo
			...
			updateViewProject();
			updateLightsCubePos(lightCubeObjects);
			animateLightsCube(lightSourcesShader, lightCube, lightCubeObjects);
	...
	auto drawScene = [&fbo, &lightSourcesShader, &lightCube, &lightCubeObjects, &woodBoxShader, &outlineShader, &skyBox, &woodCube, &skyboxShader]()
		{
			//DrawScene
			updateViewProject();
			updateLightsCubePos(lightCubeObjects);
	...
	auto drawShadow = [&simpleShadowShader,&depthMap,&shadowMap,&lightSpaceMat,&lightCubeObjects,&woodCube,&woodBoxShader,&drawScene,&terrainShader,&drawTerrain]()
		{
