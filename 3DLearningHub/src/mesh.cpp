...
void Mesh::draw(Shader& shader)
{
	...
	for (unsigned int index = 0; index < textures.size(); index++)
	{
		...
		{
			shader.setInt("Box", index); //hardcoded the name but pour l'instant 
...
void Square::drawInstanced(Shader& shader, std::string textureName, int count)
{
	shader.setInt(textureName, 0);
	glBindTexture(GL_TEXTURE_2D, texture.ID);
	glActiveTexture(GL_TEXTURE0);
...
/*--cubeForHemisphere CLASS--*/
CubeForHemisphere::CubeForHemisphere()
{
	//INDICES :
	...
	//TOP FACE
	//1st triangle : 
	pushIndices(12, {4,0,3});
	//2nd triangle : 
	pushIndices(15, {4,7,3});

	//RIGHT FACE
	//1st triangle : 
	pushIndices(18, {4,5,1});
	//2nd triangle : 
	pushIndices(21,{ 4,0,1 });

	//LEFT FACE
	//1st triangle : 
	pushIndices(24, {7,6,2});
	//2nd triangle : 
	pushIndices(27, {7,3,2});
	...
	//6
	vertices[6] = { -1.0,0.0,-1.0 };


	//7
	vertices[7] = { -1.0,1.0,-1.0 };

	setupCubeForHemisphere();
}

void CubeForHemisphere::setupCubeForHemisphere()
{
...
void CubeForHemisphere::pushIndices(int startingIndexToFill,std::array<unsigned int, 3> indicesToPush)
{
	for (int index{}; index<3;++index)
	{
		indices[startingIndexToFill + index] = indicesToPush[index];
	}
}

void CubeForHemisphere::draw(Shader& shader)
{
	glDepthFunc(GL_LEQUAL); //so skybox is visible
