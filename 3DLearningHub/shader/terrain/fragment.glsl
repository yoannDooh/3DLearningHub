...
vec3 areaFragText(Area area, vec2 textCoord, vec3 normal, vec3 viewDir, vec3 fragPos, vec3 baseColor, float shadow)
{
    ...
    //transform chunk Coord from patch range to it's own [0,1] range he j'ai trop mal explique
    ...
    {
        //transform area Coord from patch range to it's own [0,1] range tjr mal explique
        vec2 uv;
        ...
        vec3 lightning = 0.8*calcDirLight(area, sunLight, normal, viewDir, diffuse, shadow);

      
        for (int index = 0; index < POINT_LIGHTS_NB; ++index)
            lightning +=  0.5*calcPointLight(area, pointLights[index], normal, viewDir, fragPos, diffuse, shadow);
